...
public class Room implements Encodable {
    ...
    /**
     * Records whether a maintenance is ongoing.
     */
    private boolean maintenanceOngoing;

    /**
     * HazardEvaluator for this room
     */
    ...
    public void setMaintenance(boolean maintenance) {
        this.maintenanceOngoing = maintenance;
    }
     ...
     *
     * A priority list below is used to determine what the returned
     * status must be. Higher in the list has higher priority.
     *
     * If there is a TemperatureSensor in the room and it reports
     * a hazard level of 100, the room state is always EVACUATE since
     * the room is on fire.
     *
     * If there is currently a fire drill in progress,
     * the room state is always EVACUATE.
     *
     * If there is maintenance in progress and there is no fire drill or
     * fire alarm, the room state is MAINTENANCE.
     *
    ...
    public RoomState evaluateRoomState() {
        ...
        if (temperatureSensor != null && temperatureSensor.getHazardLevel() >= 100) {
    ...
    /**
     * Returns true if and only if this room is equal
     * to the other given room.
     *
     ...
     * number of sensors
     *
     * sensors (in any order). Comparison should either
     * directly or indirectly make use of each sensor's
     * equals() method.
     *
    ...
    public boolean equals(Object obj) {
        ...
        return this.roomNumber == compareRoom.getRoomNumber() &&
                this.type == compareRoom.getType() &&
                this.area == compareRoom.getArea() &&
                this.sensors.size() == compareRoom.getSensors().size() &&
                this.sensors.equals(compareRoom.getSensors());
    }

    /**
     * Returns the hash code of this room.
     *
     * Two rooms that are equal according to
    ...
    @Override
    public int hashCode() {
        return (int) (roomNumber * type.hashCode() * area *
                sensors.hashCode() * Boolean.hashCode(fireDrill) *
                Boolean.hashCode(maintenanceOngoing) *
                hazardEvaluator.hashCode());
    }
     ...
     * this room and all of its sensors.
     *
     * The format of the string to return is:
     *
     ...
     *  encodedSensorN
     *
     * where 'roomNumber' is the room's room number, 'type' is
     * the room's RoomType, 'area' is the room's area in square metres,
     * 'numSensors' is the number of sensors in the room, and 'encodedSensorX'
     * is the encoded string representation of the room's Xth sensor, sorted in
     * alphabetical order of the sensor's simple class name from 1 to N where N
     * is the number of sensors in the room. See TimedSensor.encode().
     *
     ...
     *
     * Additionally, if the room's hazard evaluator is a weighting based
     * hazard evaluator, then the lines representing the encoded sensors
     * should instead follow the format:
     *
     ...
     * Xth sensor in the alphabetically sorted list of this room's sensors,
     * from 1 to N.
     * You can assume that the order of the weightings returned by
     * WeightingBasedHazardEvaluator.getWeightings() is the same
     * as the order of sensors returned by getSensors().
     *
    ...
    public String encode() {
        boolean weightEvaluator = false;
        NumberFormat numberFormat = new DecimalFormat("###.###");
        String outputString = String.format("%d:%s:%s:%d",
                this.roomNumber,
                this.type,
                numberFormat.format(this.area),
                this.sensors.size());
        if (this.hazardEvaluator != null) {
            outputString += ":" + this.hazardEvaluator.toString();
            if (this.hazardEvaluator.toString().equals("WeightingBased")) {
                weightEvaluator = true;
            }
        }
        outputString += System.lineSeparator();
        // Evaluates if the HazardEvaluator is a WeightingBasedHazardEvaluator
        if (weightEvaluator) {
            List weights = ((WeightingBasedHazardEvaluator)
                    hazardEvaluator).getWeightings();
            for (Sensor sensor : getSensors()) {
                if (sensor instanceof Encodable) {
                    int index = getSensors().indexOf(sensor);
                    outputString += ((Encodable) sensor).encode() + "@" +
